...
최종 구현은 기존의 시각적 감성과 UX 방향성은 유지하되, 현대적인 아키텍처와 코드베이스를 기반으로 해야 한다.
...
출처: [https://goddaehee.tistory.com/597](https://goddaehee.tistory.com/597) [갓대희의 작은공간:티스토리]
...
3. `TASKS.md`
4. `SKILLS.md`
5. `design/` 폴더 내부의 모든 파일 확인

Design Reference Rules과 아키텍처를 충분히 이해하기 전까지 구현을 시작하지 않는다.

---

# Design Reference Rules

`design/` 폴더는 최종 UI의 기준 디자인으로 사용한다.

최종 구현은 `design/` 폴더에 있는 화면의 시각적 디자인, UX 흐름, 레이아웃, 컴포넌트 배치, 색감, 간격, 둥근 정도, 그림자, 마스코트, 말풍선 UI를 최대한 동일하게 재현해야 한다.

다만 기존 구현의 코드 구조와 기술 방식은 복사하지 않는다.
...
* 라이트/다크 테마의 시각적 느낌
* 한국어 타이포그래피 분위기
* 버튼, 카드, 차트, 위젯의 시각적 스타일

복사하지 말 것:

* 파일 구조
* CDN imports
* inline Babel scripts
...
즉, **디자인은 최대한 동일하게 재현하되, 코드는 현대적인 React/TypeScript 아키텍처로 새로 구현한다.**
...
Codex는 의미 있는 작업 단위마다 반드시 commit을 생성해야 한다.
